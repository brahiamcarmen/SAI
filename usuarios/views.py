...
from SAAL.models import Vivienda, SolicitudGastos, ArchivosAcueducto, Propietario, Medidores, Pqrs, RespuestasPqrs,NovedadesGenerales, CobroMatricula, Permisos, Pagos, Archivos, AsignacionExterna
...
class Inicio(LoginRequiredMixin, View):
    ...
    def get(self, request):
        try:
            ...
            totalpropietarios = Propietario.objects.count()
            totalviviendas = Vivienda.objects.count()
    ...
    def post(self, request):
        try:
            ...
            dr = datos.IdAcueducto
            acueducto = Acueducto.objects.get(IdAcueducto=dr.pk)
            form = self.form_class(request.POST, instance=acueducto)
...
class ListaViviendas(LoginRequiredMixin, View):
    ...
    def get(self, request):
        try:
            ...
            tipousuario = Permisos.objects.filter(usuid=datos, TipoPermiso='NOACLP').exists()
...
class ListaPropietarios(LoginRequiredMixin, View):
    ...
    def get(self, request):
        try:
            ...
            tipousuario = Permisos.objects.filter(usuid=datos, TipoPermiso='NOACLT').exists()
            if tipousuario is False:
                return render(request, self.template_name,
                            {
                                'usuarios': usuarios,
...
class AgregarVivienda(LoginRequiredMixin, View):
    ...
    def get(self, request):
        try:
            ...
            tipousuario = Permisos.objects.filter(usuid=usuario, TipoPermiso='NOACRP').exists()
            if tipousuario is False:
                return render(request, self.template_name,
                              {
                                  'form': form,
...
class AgregarPropietario(LoginRequiredMixin, View):
    ...
    def get(self, request):
        try:
            ...
            form = self.form_class()
            usuario = Usuario.objects.get(usuid=request.user.pk)
            tipousuario = Permisos.objects.filter(usuid=usuario, TipoPermiso='NOACRT').exists()
            if tipousuario is False:
                return render(request, self.template_name,
                              {
                                  'form': form,
...
class Reporte(LoginRequiredMixin, View):
   ...
   def get(self, request):
       try:
           ...
           tipousuario = Permisos.objects.filter(usuid=usuario, TipoPermiso='NOACRE').exists()
           if tipousuario is False:
               return render(request, self.template_name,{
...
class Busquedas(LoginRequiredMixin, View):
    ...
    def get(self, request):
        try:
            ...
            tipousuario = Permisos.objects.filter(usuid=usuario, TipoPermiso='NOACBU').exists()
            if tipousuario is False:
                return render(request, self.template_name,{
                    'notificaciones': contadorpen,
                    'listapqrs': listapqrs,
...
class GenerarNovedadDescuento(LoginRequiredMixin,View):
    login_url = '/'
    template_name = 'usuarios/generarnovedades.html'
    predio = VisualizarVivienda
    def get(self, request, IdVivienda):
        try:
            usuario = Usuario.objects.get(usuid=request.user.pk)
            ...
            return render(request, self.template_name,{
                'matricula': matricula, 'notificaciones': contadorpen,'listapqrs': listapqrs,'totalnoti': totalnoti
            })
    ...
    def post(self, request, IdVivienda):
        try:
            usuario = Usuario.objects.get(usuid=request.user.pk)
            idvivienda = request.POST.get("idvivienda")
            tiponovedad = request.POST.get("tiponovedad")
            valor = request.POST.get("valor")
            descripcion = request.POST.get("descripcion")
            vivienda = Vivienda.objects.get(IdVivienda=IdVivienda)
            ...
            if tiponovedad == DES:
                ...
                valoractual = estadocuenta.Valor
                if valoractual >= int(valor):
                    estadocuenta.Valor -= int(valor)
                    estadocuenta.save()
                    novedad = NovedadVivienda(TipoNovedad=tiponovedad,Descripcion=descripcion,Valor=valor, usuid=usuario,
                                              IdVivienda=vivienda,EstadoCuenta=estadocuenta)
                    novedad.save()
                    ver = self.predio()
                    ejecutar = ver.get(request, IdVivienda)
                    return ejecutar
                    ...
                    messages.add_message(request, messages.WARNING,'No se puede realizar el descuento valor inicial es inferior al valor indicado para el descuento')
                    return HttpResponseRedirect(reverse('usuarios:inicio'))

            elif tiponovedad == ADI:
                 estadocuenta = EstadoCuenta.objects.get(IdVivienda=IdVivienda)
                 estadocuenta.Valor += int(valor)
                 estadocuenta.save()
                 novedad = NovedadVivienda(TipoNovedad=tiponovedad, Descripcion=descripcion, Valor=valor,
                                                  usuid=usuario,
                                                  IdVivienda=vivienda, EstadoCuenta=estadocuenta)
                 novedad.save()
                 ver = self.predio()
                ...
                messages.add_message(request, messages.ERROR,'Debe elegir una opcion valida')
                return HttpResponseRedirect(reverse('usuarios:inicio'))
